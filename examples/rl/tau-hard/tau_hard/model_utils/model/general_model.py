...
def default_model() -> GeneralModel:
    from tau_hard.model_utils.model.openai import OpenAIModel
...
def model_factory(
    ...
    if platform == Platform.OPENAI:
        ...
        from tau_hard.model_utils.model.outlines_completion import OutlinesCompletionModel

        return OutlinesCompletionModel(model=model_id, base_url=base_url, temperature=temperature)
    elif platform == Platform.VLLM_CHAT:
        if base_url is None:
            raise ValueError("base_url must be provided for custom models")
        from tau_hard.model_utils.model.vllm_chat import VLLMChatModel
        ...
        if base_url is None:
            raise ValueError("base_url must be provided for custom models")
        from tau_hard.model_utils.model.vllm_completion import VLLMCompletionModel
